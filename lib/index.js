...
const DEFAULT_TOOL_NAME = "deepseek_team";
...
const Config = z.object({
	...
	maxTasks: z.number().step(1).min(1).max(Number.MAX_SAFE_INTEGER).default(8),
	maxParallel: z.number().step(1).min(1).max(Number.MAX_SAFE_INTEGER).default(6),
...
function resolveRoster(input) {
	if (input.llmProvider.length === 0) throw new Error("multiple-deepseek: llmProvider must name a registered LLM route");
	const members = (input.members ?? DEFAULT_MEMBERS).map((member) => {
		...
		if (member.model.length === 0) throw new Error(`multiple-deepseek: role "${member.role}" has an empty model`);
		if (member.label !== void 0 && member.label.length === 0) throw new Error(`multiple-deepseek: role "${member.role}" has an empty label`);
		if (member.provider !== void 0 && member.provider.length === 0) throw new Error(`multiple-deepseek: role "${member.role}" has an empty provider route`);
		if (member.persona !== void 0 && member.persona.length === 0) throw new Error(`multiple-deepseek: role "${member.role}" has an empty persona`);
		if (member.maxTokens !== void 0 && (!Number.isSafeInteger(member.maxTokens) || member.maxTokens <= 0)) throw new Error(`multiple-deepseek: role "${member.role}" maxTokens must be a positive safe integer`);
		return {
			role: member.role,
	...
	if (!roles.has(input.defaultRole)) throw new Error(`multiple-deepseek: defaultRole "${input.defaultRole}" is not a configured role (roles: ${[...roles].join(", ")})`);
	return {
...
function stopFailure(result) {
	switch (result.stopReason) {
		...
		case "aborted": return "specialist run was cancelled";
		case "error": return "specialist run failed";
		case "max-tokens": return "specialist run hit its token limit before finishing";
		case "refusal": return "specialist declined the task";
		default: return `specialist run ended abnormally (${String(result.stopReason)})`;
	}
}
...
function withPartialAnswer(error, output) {
	...
	return new TeamTaskFailure(text.length === 0 ? error : `${error}\nPartial output before the run ended:\n${text}`, output);
}
...
async function settleTeamTask(ctx, providerName, entry, buildRequest, signal) {
	...
	if (disposal.status === "rejected") return {
		...base,
		runId: String(run.id),
		status: "failed",
...
function summarizeTeam(tasks) {
	...
	if (incomplete === 0) return {
		status: "completed",
		detail: `${tasks.length} DeepSeek team tasks completed`
	};
	return {
		status: "failed",
		detail: `${incomplete} of ${tasks.length} DeepSeek team tasks did not complete`
	};
}
/** Mount the team tool on the configured subagent provider. */
function apply(ctx, config) {
	const toolName = config.toolName ?? "deepseek_team";
	if (toolName.length === 0) throw new Error("multiple-deepseek: toolName must be non-empty");
	const maxTasks = config.maxTasks ?? 8;
	if (!Number.isSafeInteger(maxTasks) || maxTasks <= 0) throw new Error("multiple-deepseek: maxTasks must be a positive safe integer");
	const maxParallel = config.maxParallel ?? 6;
	if (!Number.isSafeInteger(maxParallel) || maxParallel <= 0) throw new Error("multiple-deepseek: maxParallel must be a positive safe integer");
	const backgroundEnabled = config.enableRunInBackground !== false;
	if (config.maxDepth !== "provider-managed") assertSubagentMaxDepth(config.maxDepth);
	if (config.toolFilter !== void 0 && config.toolFilter.allow === void 0 && config.toolFilter.deny === void 0) throw new Error("multiple-deepseek: `toolFilter` is configured but names neither `allow` nor `deny` — remove the key or fill the filter");
	const roster = new MultipleDeepseekResolver(ctx, {
		llmProvider: config.llmProvider ?? "deepseek-official",
		defaultRole: config.defaultRole ?? "engineer",
		...config.members === void 0 ? {} : { members: config.members }
	});
	const roleMenuText = roleMenu(roster);
	let disposeTool;
	const mount = (provider) => {
		if (typeof config.maxDepth === "number" && !provider.capabilities.depthLimit) throw new Error(`multiple-deepseek: provider "${provider.name}" cannot enforce maxDepth (no depthLimit capability) — set maxDepth: 'provider-managed' to leave the recursion budget to the provider`);
		if (roster.listRoles().some((member) => member.persona !== void 0) && !provider.capabilities.persona) throw new Error(`multiple-deepseek: provider "${provider.name}" cannot apply specialist personas (no persona capability) — remove the roster personas or use an in-process provider`);
		if (config.toolFilter !== void 0 && !provider.capabilities.toolFilter) throw new Error(`multiple-deepseek: provider "${provider.name}" cannot scope child tools (no toolFilter capability)`);
		disposeTool = ctx.tools.register(defineTool({
			...
			output: {
				...
				render: (_args, value) => {
					if (value.kind === "background") return [{
						type: "text",
						text: `started background deepseek team ${value.jobId}`
					}];
					return [{
						type: "text",
						text: value.tasks.map((task) => {
			...
			async execute(args, exec) {
				...
				if (!parent) throw new Error("deepseek_team tool requires a calling agent (exec.agent was undefined)");
				const tasks = args.tasks;
				if (tasks.length === 0) throw new Error("multiple-deepseek: `tasks` must name at least one task");
				if (tasks.length > maxTasks) throw new Error(`multiple-deepseek: ${tasks.length} tasks exceed the configured maxTasks (${maxTasks})`);
				if (args.run_in_background === true && !backgroundEnabled) throw new Error("run_in_background is disabled for this tool instance (enableRunInBackground: false)");
						...
						jobId: jobs.start({
							...
							run: () => {
								...
								return {
									...
									done: runTeam(ctx, config.provider, entries, maxParallel, controller.signal, buildRequest).then(summarizeTeam)
								};
							}
						})
					};
...
//#endregion
export { Config, DEFAULT_LLM_PROVIDER, DEFAULT_MAX_PARALLEL, DEFAULT_MAX_TASKS, DEFAULT_MEMBERS, DEFAULT_ROLE, DEFAULT_TOOL_NAME, MultipleDeepseekResolver, UnknownRoleError, apply, inject, name, resolveRoster };
