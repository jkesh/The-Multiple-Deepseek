...
var WORK_SHELL_TOOLS = ["bash", "pwsh"];
var WORK_FS_TOOLS = ["read", "write", "edit", "glob", "grep"];
...
var SETTINGS_NAMESPACE = settingsNamespace("multiple-deepseek");
var rosterSettingsSchema = z.object({
...
var MultipleDeepseekResolver = class extends Service {
  ...
  resolve(role) {
    ...
    if (member === void 0) throw new UnknownRoleError(role, roster.members.map((spec) => spec.role));
    return member;
  }
  /**
...
var TeamDepthLimitError = class extends Error {
   ...
   */
  constructor(parentDepth, maxDepth) {
    super(
      `deepseek_team: delegation depth limit reached \u2014 this agent is already at depth ${parentDepth} and maxDepth is ${maxDepth}, so a team here would run children beyond the cap. Do not call deepseek_team again; finish the task directly in this session instead of delegating further.`
    );
...
function startFailureMessage(error, parentDepth, maxDepth) {
  if (error instanceof SubagentDepthError) {
    return `specialist delegation refused: subagent depth ${error.attemptedDepth} exceeds maxDepth ${error.maxDepth}. The delegation budget is exhausted \u2014 do not delegate further; finish the task directly.`;
  }
...
async function settleTeamTask(ctx, providerName, entry, buildRequest, signal, parent, maxDepth) {
...
async function runTeam(ctx, providerName, entries, maxParallel, signal, buildRequest, parent, maxDepth) {
...
function workToolCoverage(ctx, parent) {
  return {
    shell: WORK_SHELL_TOOLS.filter((name2) => ctx.tools.get(name2, parent) !== void 0),
    fs: WORK_FS_TOOLS.filter((name2) => ctx.tools.get(name2, parent) !== void 0)
  };
}
function apply(ctx, config) {
  ...
  const mount = (provider) => {
    ...
    disposeTool = ctx.tools.register(defineTool({
      name: toolName,
      description: "Coordinate a team of multiple DeepSeek specialists. Each task names a specialist role, and the role selects its DeepSeek model and specialist instructions automatically \u2014 name the kind of work, never a model. Tasks run in parallel; this call waits for every member and returns one result per task. Members inherit this session's tool set, so a team that must read, edit, or run code requires a session with file and shell tools; the call refuses upfront when none are visible (set `requireWorkTools: false` for tool-free parallel reasoning). A delegation-depth limit prevents runaway nested teams: if the calling agent is already at or above the configured maxDepth, the call is rejected before any member starts." + (backgroundEnabled ? " Set `run_in_background: true` to run the whole team as a background job and collect it with `job_output`." : ""),
      parameters: {
        tasks: {
          type: "array",
          required: true,
      ...
      async execute(args, exec) {
        ...
        if (config.requireWorkTools !== false) {
          ...
          if (coverage.shell.length === 0 && coverage.fs.length === 0) {
            throw new Error(
              `deepseek_team: team members inherit the calling session's tools, and this session exposes no file or shell workbench (shell tools visible: ${coverage.shell.join(", ") || "none"}; file tools visible: ${coverage.fs.join(", ") || "none"}). Switch this session to a preset that mounts file/shell tools (the shipped team-mode preset mounts pwsh/bash, read/write/edit, and glob/grep), or set \`requireWorkTools: false\` to allow tool-free parallel reasoning.`
            );
          }
        }
        ...
        if (args.run_in_background === true) {
          ...
          const id = jobs.start({
            ...
            run: () => {
              ...
              const done = runTeam(ctx, config.provider, entries, maxParallel, controller.signal, buildRequest, parent, config.maxDepth).then(summarizeTeam);
              return {
                cancel: (reason) => {
